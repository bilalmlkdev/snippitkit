/**
 * SnippetKit
 * Copyright (c) 2026, Bilal Malik (bilalmlkdev)
 * All rights reserved.
 */
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function detectLanguageFromText(text) {
    ...
    if (/\b(def\s+\w+\(|from\s+\w+|import\s+\w+|print\(|self\b)/.test(t))
      return "language-python";
    if (
      /\b(public\s+class|System\.out|println|package\s+[a-z0-9_.]+)\b/.test(t)
    )
      return "language-java";
    if (/\b#include\s+<|int\s+main\s*\(|std::\w+/.test(t))
      return "language-cpp";
    if (/\busing\s+system;|namespace\s+\w+;|Console\.WriteLine/.test(t))
  ...
  if (getComputedStyle(codePanel).position === "static")
  ...
  function populateSelects({ themes, languages, fonts }) {
    ...
    const fontSelect = document.querySelector(
      '.custom-select[data-name="fonts"] .select-options',
    );
  ...
  loadData()
    ...
    .catch((err) => {
      ...
      document
        .querySelectorAll(".custom-select .trigger-value")
        .forEach((el) => {
  ...
  function applyLanguageClass(langOverride) {
    let cls =
      langOverride ||
      (currentLanguage && currentLanguage.trim() ? currentLanguage : "");
    if (!cls && editor.value.trim()) {
      cls = detectLanguageFromText(editor.value);
    }
    codeBlock.className = cls.includes("language-")
      ? cls
      : cls
        ? `language-${cls}`
        : "";
  ...
  window.applyLanguage = (input) => {
    ...
    if (typeof input === "string") {
      ...
      else if (prismLangMap[input.toLowerCase()])
        cls = prismLangMap[input.toLowerCase()];
      else {
  ...
  borderRadiusInput?.addEventListener("input", () => {
    if (panelScreen)
      panelScreen.style.borderRadius = `${borderRadiusInput.value}px`;
  });
  ...
  modeToggleInput?.addEventListener("change", () => {
    ...
    document
      ...
      ?.classList.toggle("template-name-active");
  ...
  function initCustomSelect(root) {
    ...
    function positionMenu() {
      if (isPortaled) {
        ...
        if (!openUp)
          optionsList.style.top = `${Math.round(rootRect.bottom + 6)}px`;
        else
          optionsList.style.bottom = `${Math.round(window.innerHeight - rootRect.top + 6)}px`;
    ...
    function openSelect() {
      ...
      if (needsPortal && !isPortaled) {
        ...
        if (originalNextSibling)
          originalParent.insertBefore(optionsList, originalNextSibling);
  ...
  function updateRangeVisual(r) {
    ...
    const fillColor =
      r.dataset.fillColor ||
      getComputedStyle(r).getPropertyValue("--fill-color") ||
      "#ddd";
    const trackColor =
      r.dataset.trackColor ||
      getComputedStyle(r).getPropertyValue("--track-color") ||
      "#000";
